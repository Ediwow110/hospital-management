...
// ===== USER MANAGEMENT =====
const demoUsersStore=[{id:'u-admin',name:'A. Reyes',username:'areyes',email:'a.reyes@hospital.local',role:'super_admin',branch:'Main Branch',status:'active'},{id:'u-reception',name:'R. Cruz',username:'rcruz',email:'r.cruz@hospital.local',role:'receptionist',branch:'Main Branch',status:'active'},{id:'u-cashier1',name:'C. Santos',username:'csantos',email:'c.santos@hospital.local',role:'cashier',branch:'Main Branch',status:'active'}];
function showUserListScreen(){const tb=document.getElementById('users-table-body');tb.innerHTML='';demoUsersStore.forEach(u=>{const tr=document.createElement('tr');tr.innerHTML=`<td>${u.name}</td><td>${u.email}</td><td>${roleLabel(u.role)}</td><td>${u.branch}</td><td>${u.status==='active'?'<span class="badge bg-success">Active</span>':'<span class="badge bg-secondary">Inactive</span>'}</td><td><button class="btn btn-sm btn-outline-primary" onclick="editUser('${u.id}')"><i class="bi bi-pencil"></i> Edit</button> ${u.status==='active'?`<button class="btn btn-sm btn-outline-warning" onclick="deactivateUser('${u.id}')">Deactivate</button>`:`<button class="btn btn-sm btn-outline-success" onclick="reactivateUser('${u.id}')">Reactivate</button>`}</td>`;tb.appendChild(tr);});}
function checkPrivilegedRole(r,s){const w=document.getElementById(s+'-role-warning');w.style.display=['super_admin','branch_manager'].includes(r)?'block':'none';}
function saveNewUser(){const fn=document.getElementById('create-fullname').value.trim(),un=document.getElementById('create-username').value.trim(),em=document.getElementById('create-email').value.trim(),pw=document.getElementById('create-password').value,ro=document.getElementById('create-role').value,br=document.getElementById('create-branch').value,st=document.getElementById('create-status').value;if(!fn||!un||!em||!ro||!pw)return alert('⚠ Fill all required fields');if(pw.length<8)return alert('⚠ Password min 8 chars');if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(em))return alert('⚠ Invalid email');if(demoUsersStore.find(u=>u.username===un))return alert('⚠ Username exists');if(demoUsersStore.find(u=>u.email===em))return alert('⚠ Email exists');demoUsersStore.push({id:'u-'+Date.now(),name:fn,username:un,email:em,role:ro,branch:br,status:st});document.getElementById('user-create-form').reset();document.getElementById('create-role-warning').style.display='none';alert('✓ User created!');showScreen('users');}
function editUser(id){const u=demoUsersStore.find(x=>x.id===id);if(!u)return;document.getElementById('edit-user-id').value=u.id;document.getElementById('edit-fullname').value=u.name;document.getElementById('edit-username').value=u.username;document.getElementById('edit-email').value=u.email;document.getElementById('edit-role').value=u.role;document.getElementById('edit-branch').value=u.branch;document.getElementById('edit-status').value=u.status;document.getElementById('edit-original-role').value=u.role;document.getElementById('edit-original-branch').value=u.branch;document.getElementById('edit-original-status').value=u.status;document.getElementById('edit-original-email').value=u.email;checkPrivilegedRole(u.role,'edit');showScreen('user-edit');}
function saveUserEdit(){const id=document.getElementById('edit-user-id').value,fn=document.getElementById('edit-fullname').value.trim(),em=document.getElementById('edit-email').value.trim(),ro=document.getElementById('edit-role').value,br=document.getElementById('edit-branch').value,st=document.getElementById('edit-status').value,or=document.getElementById('edit-original-role').value,ob=document.getElementById('edit-original-branch').value,os=document.getElementById('edit-original-status').value,oe=document.getElementById('edit-original-email').value;if(!fn||!em||!ro)return alert('⚠ Fill all required fields');if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(em))return alert('⚠ Invalid email');const sc=[];if(ro!==or)sc.push('role');if(br!==ob)sc.push('branch');if(st!==os)sc.push('status');if(em!==oe)sc.push('email');if(sc.length>0){showReasonModal(`Changing: ${sc.join(', ')}. Reason required.`,()=>performUserUpdate(id,fn,em,ro,br,st));}else{performUserUpdate(id,fn,em,ro,br,st);}}
function performUserUpdate(id,fn,em,ro,br,st){const u=demoUsersStore.find(x=>x.id===id);if(!u)return;u.name=fn;u.email=em;u.role=ro;u.branch=br;u.status=st;alert('✓ User updated!');showScreen('users');}
function deactivateUser(id){showReasonModal('Deactivating user. Provide reason:',()=>{const u=demoUsersStore.find(x=>x.id===id);if(u){u.status='inactive';alert('✓ User deactivated!');showUserListScreen();}});}
function reactivateUser(id){showReasonModal('Reactivating user. Provide reason:',()=>{const u=demoUsersStore.find(x=>x.id===id);if(u){u.status='active';alert('✓ User reactivated!');showUserListScreen();}});}
let reasonModalCallback=null;
function showReasonModal(txt,cb){document.getElementById('reason-prompt').textContent=txt;document.getElementById('reason-text').value='';document.getElementById('reason-error').style.display='none';reasonModalCallback=cb;new bootstrap.Modal(document.getElementById('reasonModal')).show();}
document.getElementById('reason-submit-btn')?.addEventListener('click',function(){const r=document.getElementById('reason-text').value.trim(),e=document.getElementById('reason-error');if(r.length<10){e.style.display='block';return;}e.style.display='none';bootstrap.Modal.getInstance(document.getElementById('reasonModal')).hide();if(reasonModalCallback){reasonModalCallback();reasonModalCallback=null;}});
const origShowScreen=window.showScreen;if(origShowScreen){window.showScreen=function(s){origShowScreen(s);if(s==='users')showUserListScreen();};}
